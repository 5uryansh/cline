            ...
            switch (type) {
                ...
                case "completion":
                    ...
                    if (payload.fast) {
                        ...
                        exec(`git add . && git commit -m "${commitMessage}"`, { cwd: payload.repoPath }, (err, stdout, stderr) => {
                            if (err) {
                            ...
                            exec('git rev-parse --abbrev-ref HEAD', { cwd: payload.repoPath }, (err, branchName) => {
                                if (err) {
                                    reject(new Error(`Failed to get current branch name: ${err.message}`));
                                    return;
                                }
                                exec(`git push --set-upstream origin ${branchName.trim()}`, { cwd: payload.repoPath }, (err) => {
                                    if (err) {
